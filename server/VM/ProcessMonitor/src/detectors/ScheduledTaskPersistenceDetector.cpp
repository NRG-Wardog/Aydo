...
std::vector<Finding> ScheduledTaskPersistenceDetector::evaluate(const NormalizedEvent &ne,
                                                                ThreadCaches &) {
  if (!isMatch(ne)) {
    return {};
  }
...
bool ScheduledTaskPersistenceDetector::isMatch(const NormalizedEvent &ne) const {
  return ThreadHelpers::looksLikeScheduledTaskPersistence(ne);
}

bool ScheduledTaskPersistenceDetector::_isDuplicate(
    DWORD srcPid,
    DWORD tgtPid,
    DWORD tid,
...
Finding ScheduledTaskPersistenceDetector::_buildFinding(const NormalizedEvent &ne,
                                                        DWORD srcPid,
                                                        DWORD tgtPid,
                                                        DWORD tid) const {
  nlohmann::json ev;
  ...
  ev["tgtPid"] = tgtPid;
  ev["tid"] = tid;
  if (auto s = ThreadHelpers::getStr(ne, "TaskName")) {
    ev["task"] = *s;
  }
  ...
  Finding finding;
  finding.type = "ScheduledTaskPersistence";
